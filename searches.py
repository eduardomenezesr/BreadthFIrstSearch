...
def A_star(file_name):
    ...
    while(not all_visited and not finished):
        ...
        if v.getEdges():
            ...
            for edge in edges:
                # we will check if there is already a vertex with the same
                # name on the frontier list, if not then the vertex will be
                # added to the frontier list.
                ed_exist = False
                for f in range(len(frontier)):
                    if(edge.getTo() == frontier[f].getName()):
                        ed_exist = True
                if(not ed_exist):
                    ...
                    # for the heuristic of the state stored in the variable
                    # v_aux
                    file_object.seek(0)
                    for line in file_object:
